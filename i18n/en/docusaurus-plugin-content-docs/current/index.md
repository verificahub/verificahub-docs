---
id: index
title: Documentation
slug: /
sidebar_position: 1
---

# Verificahub Documentation

Verificahub is a single platform for verifying user phone numbers: reverse flash-call, flash-call, voice, SMS, Telegram and MAX through one API.

## Sections

### Getting started
- [Overview and onboarding](./start)

### API integration
- [Overview](./integration/overview)
- [Verification flows](./integration/flows)
- [Webhooks](./integration/webhooks)
- [Best practices](./integration/best_practices)

### Use-case guides
- [Methods and pricing](./guides/methods)
- [Reverse flash-call](./guides/reverse_flash_call)
- [Code verification (Telegram, SMS, Flash Call)](./guides/code_verification)
- [Balance and usage](./guides/balance_and_usage)
- [Statuses and errors](./guides/errors_and_statuses)

### FAQ
- [FAQ](./faq)

### Legal
- [Public offer](./legal/public_offer)
- [Privacy policy](./legal/privacy_policy)
- [Terms of use](./legal/terms_of_use)
- [Personal data consent](./legal/personal_data_consent)

:::note
Legal documents are published in Russian only and are legally binding in that version.
:::
