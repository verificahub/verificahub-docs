...
| `flash_call` | Мы звоним-сбросом на номер пользователя; код — последние цифры входящего номера | Да — через `/v1/verify/check` |
| `sms` | Код приходит в SMS, пользователь вводит его у вас | Да — через `/v1/verify/check` |
| `telegram_otp` | Код приходит в Telegram, пользователь вводит его у вас | Да — через `/v1/verify/check` |
| `max_bot` | Код приходит в мессенджере MAX — после того как пользователь откроет бота по `deep_link` и поделится номером; вводит его у вас | Да — через `/v1/verify/check` |
| `mts_id` | Оператор МТС «Мобильный ID» присылает подтверждение на SIM; при откате — код по SMS | Только при откате на SMS-OTP |

`telegram_otp`, `max_bot` и `mts_id` доступны, если включены для вашего аккаунта. Актуальные цены по каждому
