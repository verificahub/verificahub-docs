...
При неуспешной инициации (`4xx`/`5xx` на `POST /v1/verify`) списание не происходит. Для `max_bot`
списание — по факту доставки: если пользователь не открыл бота или не поделился номером, код не
доставлен и оплата не взимается.
